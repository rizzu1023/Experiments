...
void insafter(node *q,int no)            //modify
{
	int loc,k;
	node *temp,*ptr;                                    //delete old
	temp=q;
	ptr=(node*)malloc(sizeof(node*));
	ptr->data=no;
	...
	temp->prev->next=ptr;                      //new
	ptr->next=temp->prev;                   //modify
    ptr->next=temp;                            //new
    temp->prev=ptr;                            //new
	printf("\nELEMENTS OF LINKLIST AFTER INSERTION\n");
	print(q);
